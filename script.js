...
async function generatePrompt() {
    ...
    resultBox.innerHTML = "<p class='text-gray-500'>Generating...</p>";
...
// 3. Content Manager & Master Prompt Auto-Builder
async function generateContentStrategy() {
    ...
    btn.disabled = true;
    btn.innerText = "Researching & Engineering Prompts... (15-20s)";
    resultBox.innerHTML = "<p class='text-blue-500 font-semibold animate-pulse'>Analyzing niche and engineering elite 7-Phase Master Prompts... Please wait.</p>";

    const strategyPrompt = `Act as an Elite Social Media Manager and Advanced AI Prompt Engineer. The user's niche is: "${niche}". Provide a beautifully formatted Markdown response divided into TWO main sections.
    ## SECTION 1: Niche Strategy
    1. Target Audience Analysis
    2. Top 15 Viral Keywords & SEO Tags
    3. 7-Day Content Calendar (Markdown Table)
    ## SECTION 2: Auto-Generated 7-Phase Master Prompts
    Based on the exact niche "${niche}", generate TWO highly detailed "7-Phase Master Prompts". One for IMAGES and One for SHORT VIDEOS. Put EACH inside a markdown code block (\`\`\`). Follow the exact 7-Phase structure: Role, Mission, Style, Categories, Idea Gen, Format, AI Prompt, Caption, Overlay, Hashtags, Strategy.`;
    ...
    try {
        ...
        btn.disabled = false;
        btn.innerText = "Generate Strategy & Master Prompts";
    }
}
...
function loadStore() {
    ...
    listDiv.innerHTML = "";
    if(store.length === 0) {
        listDiv.innerHTML = "<p class='text-gray-500 italic'>No saved items yet.</p>";
        return;
    }
    store.forEach((item) => {
        const formattedContent = marked.parse(item.content);
        listDiv.innerHTML += `
            <div class="p-6 border rounded-lg bg-white shadow-md relative glass-panel">
                <div class="flex items-center mb-4 border-b pb-2">
                    <span class="text-xs font-bold text-white bg-blue-600 px-3 py-1 rounded-full">${item.type}</span>
                    <span class="text-xs text-gray-500 ml-3">${item.date}</span>
                </div>
